...
use super::{direction::Direction, over_map::{OverMap, OverMapTile}, grid::{Grid2D, Coord}, terrain::{TerrainMap, TerrainType}};
	...
	match dir {
		Some(dir) => {
			...
			unit.order = match terrain.get(next_position) {
				...
				TerrainType::Grass | TerrainType::Sand => if terrain.get(unit.position) == TerrainType::Water {
					MoveOrder::Swim
				} else {
					MoveOrder::Walk
				},
...
pub fn move_units(
	...
	mut query: Query<(Entity, &mut UnitPosition, &mut TextureAtlasSprite, &mut Transform)>
) {
